...
export function tinhTienTK(lop, giaVatLieu) {
    ...
    for (let i = 0; i < lop.length; i++) {
        if (lop[i].chatLieu.substring(0, 4) === "theu") {
            tongtien = tongtien + Math.round(lop[i].chieuDai * lop[i].chieuRong * 2.54 * 2.54 * 27000 * lop[i].soMatIn * tienCongPhut / (700 * 100))
            sttTheu = true;
        }
    }
    if (sttTheu) {
        if (tongtien < 15000) return 15000
        return tongtien
    }
...
export function tinhTienDIen(lop, giaVatLieu) {
    ...
    tongtien = (5 * 2400 * tienIn) / (tienCongPhut * 60) + (3 * 2400 * tienCat) / (tienCongPhut * 60);
